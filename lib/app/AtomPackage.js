...
export default class AtomPackage {
  ...
  activate(state) {
    ...
    this.atom = new AtomHelper(this.element);
    ...
    this.atom.on('refreshTabs', this.dispatcher(actions.refreshTabs));
  }
  ...
  openTerminal() {
    this.atom.runCommand('term3:open');
  }

  reset() {
    this.state = {};
    this.atom.runCommand('window:reload');
  }
